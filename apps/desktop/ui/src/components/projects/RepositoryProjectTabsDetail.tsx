...
export function RepositoryProjectDetail({
  ...
  const loadUnityAdapterSettingsEffect = useEffectEvent(async () => {
    ...
    try {
      ...
      startTransition(() => {
        ...
        setUnityAdapterSettingsError(buildProjectSaveErrorMessage(loadError));
        setIsLoadingUnityAdapterSettings(false);
      });
    }
  });
  ...
  const handleSavePublishCredential = useEffectEvent(
    async (destinationId: string, input: SaveSecretCredentialInput) => {
      try {
        ...
        throw new Error(buildProjectSaveErrorMessage(saveCredentialError));
      }
    },
  );
  ...
  const handleSaveProject = useEffectEvent(async () => {
    ...
    try {
      ...
      startTransition(() => {
        setSaveMessage(`Saved changes for ${repository.repository_name}.`);
      });
  ...
  const projectSourceDisplay = buildProjectSourceDisplay(projectSourceInput);
  ...
  if (isLoading) {
    return (
      <div className="project-detail-shell">
        <FocusPageFrame
          description="Inspect and edit project definition steps with the same shared surfaces used by creation."
          eyebrow="Project"
  ...
  return (
    <div className="project-detail-shell">
      <FocusPageFrame
        ...
        eyebrow={formatProjectKindLabel(draft.projectKind)}
        summary={
          <MetaRow>
            ...
            <MetaItem label="Mode">
              {resolveProjectSourceModeSummary(projectSourceInput)}
            </MetaItem>
            <MetaItem label="Targets">{draft.buildTargets.length}</MetaItem>
            <MetaItem
...
function buildStepSummary(
  ...
  switch (stepKey) {
    case "identity":
      return (
        <MetaRow>
          <MetaItem label="Name">{draft.name.trim() || "Unnamed"}</MetaItem>
          <MetaItem label="Kind">
    ...
    case "access":
      return draft.projectKind === "repository" ? (
        <MetaRow>
          <MetaItem label="Visibility">{draft.repositoryVisibility}</MetaItem>
          <MetaItem label="Poll">{`${draft.pollingIntervalSeconds.trim() || "0"}s`}</MetaItem>
          <MetaItem label="Access">{repositoryAccessSummary}</MetaItem>
        </MetaRow>
      ) : (
        <MetaRow>
          <MetaItem label="Workspace">
            {draft.localPath.trim() ? "Configured" : "Missing"}
          </MetaItem>
        </MetaRow>
      );
    case "targets":
      return (
        <MetaRow>
          <MetaItem label="Targets">{draft.buildTargets.length}</MetaItem>
          <MetaItem label="Executable">
            {draft.unityExecutablePath.trim() ? "Configured" : "Pending"}
          </MetaItem>
        </MetaRow>
      );
    case "publish":
    ...
    case "review":
      return (
        <MetaRow>
          <MetaItem label="Ready">Review before save</MetaItem>
        </MetaRow>
      );
  }
}
...
function buildValidationState(input: {
  ...
  const identity = validateIdentityStep(input.draft, input.inventory);
  ...
  const paths = validatePathStep(input.draft);
