...
struct OrphanBuildProcessTerminationReport {
...
fn git_command() -> Command {
    #[cfg(test)]
    if let Some(path) = std::env::var_os("HANDY_UNITY_BUILDER_TEST_GIT_EXECUTABLE") {
...
mod tests {
    ...
    fn select_orphan_build_process_roots_matches_unity_process_by_workspace_path() {
        let interrupted_builds = vec![InterruptedBuildRecoveryRecord {
            ...
            workspace_path: String::from(
                r"D:\Users\gabao\RevolutionsHandyUnityBuilderWorkspace\runs\build-run-3-attempt-25496-1778529831533390600",
            ),
            log_path: Some(String::from(
                r"D:\Users\gabao\RevolutionsHandyUnityBuilderWorkspace\runs\build-run-3-attempt-25496-1778529831533390600\logs\03-unity-build.log",
            )),
            interruption_kind: String::from(RECOVERY_INTERRUPTION_KIND_SYSTEM),
            interruption_message: String::from(
                "build attempt interrupted after an unexpected runtime interruption",
        ...
        let processes = vec![
            ObservedProcess {
                ...
                name: String::from("runtime-bin.exe"),
                command_line: String::from(
                    r#""C:\Users\gabao\projects\handy-unity-builder\target\debug\runtime-bin.exe" serve"#,
                ),
            },
            ObservedProcess {
                pid: 4436,
                ...
                command_line: String::from(
                    r#""C:\Program Files\Unity\Hub\Editor\6000.4.3f1\Editor\Unity.exe" -batchmode -quit -nographics -logFile D:\Users\gabao\RevolutionsHandyUnityBuilderWorkspace\runs\build-run-3-attempt-25496-1778529831533390600\logs\03-unity-build.log -projectPath D:\Users\gabao\RevolutionsHandyUnityBuilderWorkspace\runs\build-run-3-attempt-25496-1778529831533390600\source"#,
                ),
            },
            ObservedProcess {
                pid: 9999,
                ...
                command_line: String::from(
                    r#"powershell -NoProfile -Command \"Get-Content -Tail 40 'D:\Users\gabao\RevolutionsHandyUnityBuilderWorkspace\runs\build-run-3-attempt-25496-1778529831533390600\logs\03-unity-build.log'\""#,
                ),
            },
        ];
    ...
    impl FakeGitFixture {
        fn install(root: &Path, required_token: &str, unity_version: &str) -> Self {
            ...
            let original_git_executable =
                std::env::var_os("HANDY_UNITY_BUILDER_TEST_GIT_EXECUTABLE");
            ...
            std::env::set_var(
                "HANDY_UNITY_BUILDER_TEST_GIT_EXECUTABLE",
                &script_path,
            );
    ...
    impl Drop for FakeGitFixture {
        fn drop(&mut self) {
            if let Some(path) = &self.original_git_executable {
                std::env::set_var("HANDY_UNITY_BUILDER_TEST_GIT_EXECUTABLE", path);
            } else {
                std::env::remove_var("HANDY_UNITY_BUILDER_TEST_GIT_EXECUTABLE");
            }
    ...
    fn test_root(label: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "handy-unity-builder-runtime-store-{label}-{}",
            std::process::id()
        ))
    }
    }