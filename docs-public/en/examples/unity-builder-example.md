# Unity Builder Example

Use this example when you need a ready-to-adapt script for the build methods
referenced by HGP build targets.

Save it under an `Assets/Editor` folder in your Unity project, then keep the
suggested method names or rename them to match the names configured in your
build targets.

Download the raw file: [UnityBuilderExample.cs](../assets/code/UnityBuilderExample.cs)
